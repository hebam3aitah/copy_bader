...
export default function AdminIssuesPage() {
  ...
  const exportToPDF = () => {
    try {
      ...
      // عنوان الملف
      doc.setFontSize(18);
      doc.text("تقرير البلاغات", doc.internal.pageSize.width / 2, 15, { align: 'center' });
      
      // تاريخ التقرير
      doc.setFontSize(12);
      doc.text(`تاريخ التقرير: ${formatDate(new Date())}`, doc.internal.pageSize.width - 20, 25, { align: 'right' });
      ...
      // رؤوس الجدول
      const headers = [
        ["تاريخ الإنشاء", "الحالة", "رقم الهاتف", "اسم المبلغ", "الخطورة", "الموقع", "نوع المشكلة"]
      ];
      ...
      doc.autoTable({
        ...
        headStyles: { fillColor: [49, 18, 75], textColor: [255, 255, 255] },
        alternateRowStyles: { fillColor: [245, 245, 245] },
        columnStyles: { 0: { cellWidth: 35 } },
      });
      
      // حفظ الملف
      doc.save(`بلاغات-${format(new Date(), 'yyyy-MM-dd')}.pdf`);
      
      toast.success('تم تصدير البيانات بنجاح إلى PDF');
    } catch (error) {
      toast.error('فشل في تصدير البيانات إلى PDF');
  ...
  return (
      ...
      <div className="container mx-auto p-6">
        ...
        <div className="bg-white rounded-lg shadow-md p-4 mb-6">
          ...
          <div className="flex flex-col lg:flex-row justify-between items-center gap-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full lg:w-auto">
              <div>
                <label className="font-medium text-gray-700 block mb-1">من تاريخ</label>
                <input
                  type="date"
              ...
              <div>
                <label className="font-medium text-gray-700 block mb-1">إلى تاريخ</label>
                <input
                  type="date"
                  className="w-full px-4 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-[#31124b] focus:border-transparent"
                  value={dateRange.end}
        ...
        <div className="flex justify-between items-center mb-4">
          <div className="bg-white px-4 py-2 rounded-lg shadow-sm">
            ...
            <span className="font-bold text-[#31124b]">{filteredIssues.length}</span>
            <span className="text-gray-600 mx-2">|</span>
            <span className="text-gray-600">عرض: </span>
            <span className="font-bold text-[#31124b]">{currentItems.length}</span>
          </div>
          
          <div className="flex gap-2 items-center">
            <span className="text-gray-600 text-sm">عدد العناصر في الصفحة:</span>
            <select
              value={itemsPerPage}
              onChange={(e) => {
        ...
        {!loading && filteredIssues.length > 0 && (
          <div className="flex items-center justify-between bg-white mt-4 p-4 rounded-lg shadow-md">
            <div className="text-sm text-gray-600">
              عرض {indexOfFirstItem + 1} - {Math.min(indexOfLastItem, filteredIssues.length)} من {filteredIssues.length} بلاغ
            </div>
            <div className="flex items-center gap-2">
              <button
                onClick={() => paginate(currentPage > 1 ? currentPage - 1 : 1)}
                disabled={currentPage === 1}
                className={`px-3 py-1 rounded-md border ${currentPage === 1 ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : 'bg-white text-[#31124b] hover:bg-gray-50'}`}
              >
                السابق
              </button>
              ...
              {Array.from({ length: totalPages }, (_, i) => i + 1).map(number => (
                <button
                  ...
                  className={`w-8 h-8 rounded-md ${currentPage === number ? 'bg-[#31124b] text-white' : 'bg-white text-[#31124b] hover:bg-gray-50'}`}
                >
                  {number}
                </button>
              ))}
              ...
              <button
                ...
                className={`px-3 py-1 rounded-md border ${currentPage === totalPages ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : 'bg-white text-[#31124b] hover:bg-gray-50'}`}
              >
                التالي
              </button>
            </div>
    ...
    >
        ...
        {/* عرض جميع الصور */}
        {showAllImages.length > 0 && (
          <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4" onClick={() => setShowAllImages([])}>
            <div className="relative max-w-6xl w-full bg-white rounded-lg p-4 max-h-[90vh] overflow-y-auto">
              ...
              <h3 className="text-xl font-bold text-[#31124b] mb-4 text-center">معرض الصور</h3>
...
function FilterButton({ children, active, onClick, count, color }) {
  return (
    <button
      className={`px-3 py-1 rounded-md text-sm font-medium transition ${active ? 'text-white' : 'text-gray-700 hover:bg-gray-100'}`}
...
function SeverityBadge({ level }) {
  ...
  return (
    <span className={`px-2 py-1 rounded-full text-xs font-medium ${severityStyles[level] || 'bg-gray-100 text-gray-800'}`}>
