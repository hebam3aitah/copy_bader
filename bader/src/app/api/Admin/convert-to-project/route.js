...
export async function POST(req) {
  try {
    ...
    if (!categoryId && issue.problemType) {
      ...
      if (category) {
        ...
        console.log("تم العثور على/إنشاء التصنيف وتحديث البلاغ:", category.name);
      } else {
    ...
    if (!categoryId) {
      return NextResponse.json({ error: 'البلاغ لا يحتوي على تصنيف (category) ولم نستطع إيجاد أو إنشاء تصنيف' }, { status: 400 });
    }
    ...
    if (Array.isArray(issue.images)) {
      ...
      processedImages = issue.images.map(img => {
  ...
  user: issue.reportedBy || issue.createdBy,
    ...
    return NextResponse.json({ success: true, projectId: project._id });
      ...
      return NextResponse.json({ error: projectError.message || 'فشل في إنشاء المشروع' }, { status: 500 });
    }
