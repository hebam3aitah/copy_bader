...
export default function ProjectDetails() {
  ...
  const handleLike = async () => {
    try {
      ...
      if (!response.ok) throw new Error('فشل في تحديث الإعجاب');
      setLiked(!liked);
      setProject(prev => ({ ...prev, likes: liked ? (prev.likes || 1) - 1 : (prev.likes || 0) + 1 }));
    } catch (err) {
  ...
  const handleShare = (platform) => {
    ...
    switch(platform) {
      case 'twitter':
        window.open(`https://twitter.com/intent/tweet?text=تعرف على مشروع ${project?.title || ''} من بادر&url=${shareUrl}`);
        break;
      case 'facebook':
        window.open(`https://www.facebook.com/sharer/sharer.php?u=${shareUrl}`);
        break;
      case 'whatsapp':
        window.open(`https://wa.me/?text=تعرف على مشروع ${project?.title || ''} من بادر: ${shareUrl}`);
        break;
      case 'copy':
    ...
    return (
      <div className="flex justify-center items-center min-h-screen bg-gray-50">
        <div className="flex flex-col items-center">
          ...
          <p className="text-xl font-semibold text-[#31124b]">جاري تحميل المشروع...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="flex justify-center items-center min-h-screen bg-gray-50">
        <div className="text-center p-8 bg-white rounded-xl shadow-lg max-w-md">
          ...
          <h2 className="text-2xl font-bold text-[#31124b] mb-2">عذراً، حدث خطأ!</h2>
    ...
    return (
      <div className="flex justify-center items-center min-h-screen bg-gray-50">
        <div className="text-center p-8 bg-white rounded-xl shadow-lg max-w-md">
          ...
          <h2 className="text-2xl font-bold text-[#31124b] mb-2">لم يتم العثور على المشروع</h2>
          <p className="text-gray-600 mb-4">المشروع الذي تبحث عنه غير موجود أو تم حذفه</p>
          <Link 
            href="/projects"
            className="inline-block bg-[#31124b] text-white px-6 py-2 rounded-lg hover:bg-[#2a0f40] transition-colors"
          >
    ...
    <div className="min-h-screen bg-gray-50" dir="rtl">
      ...
      <div className="relative w-full h-[60vh]">
        ...
        <div className="absolute bottom-0 right-0 p-8 z-20">
          <h1 className="text-4xl font-bold text-white mb-2">{project.title || 'مشروع بدون عنوان'}</h1>
          <div className="flex items-center gap-4 text-white">
            <div className="flex items-center">
              <FiUsers className="ml-1" />
              <span>{project.volunteers || 0} متطوع</span>
            </div>
            <div className="flex items-center">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2">
            ...
            >
              ...
              <p className="text-gray-700 mb-6 leading-relaxed">{project.description || 'لا يوجد وصف متاح لهذا المشروع.'}</p>
                  ...
                  <div className="relative">
                    ...
                    <AnimatePresence>
                      {showShareOptions && (
                        ...
                        >
                          <button onClick={() => handleShare('facebook')} className="block w-full text-right py-2 px-3 hover:bg-gray-100 rounded">فيسبوك</button>
                          <button onClick={() => handleShare('twitter')} className="block w-full text-right py-2 px-3 hover:bg-gray-100 rounded">تويتر</button>
                          <button onClick={() => handleShare('whatsapp')} className="block w-full text-right py-2 px-3 hover:bg-gray-100 rounded">واتساب</button>
                          <button onClick={() => handleShare('copy')} className="block w-full text-right py-2 px-3 hover:bg-gray-100 rounded">نسخ الرابط</button>
                        </motion.div>
                      )}
                    </AnimatePresence>
                  </div>
              ...
              <div className="relative mb-8">
                <div className="w-full h-80 relative rounded-xl overflow-hidden">
                  <Image 
                    src={project.images && project.images[currentImageIndex] ? project.images[currentImageIndex] : PLACEHOLDER}
                ...
                <div className="flex justify-center mt-4 gap-2">
                  {(project.images && project.images.length > 0 ? project.images : [PLACEHOLDER]).map((_, index) => (
                    <button 
                      key={index}
                      onClick={() => setCurrentImageIndex(index)} 
                      className={`w-3 h-3 rounded-full transition-colors ${
                        currentImageIndex === index ? 'bg-[#fa9e1b]' : 'bg-gray-300'
                      }`}
                    />
                  ))}
                </div>
              ...
              <div className="space-y-4">
                {comments.length === 0 && <div className="text-gray-400">لا توجد تعليقات بعد.</div>}
                  ...
                  <div key={comment.id} className="bg-gray-50 rounded-lg p-4">
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-semibold text-[#31124b]">{comment.user || 'مستخدم'}</span>
                      </div>
          ...
          <div className="lg:col-span-1">
            ...
            >
              <h3 className="text-xl font-bold mb-4 text-[#31124b]">معلومات المشروع</h3>
              
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <span className="text-gray-600">الحالة</span>
                  <span className={`font-semibold ${project.status === 'مكتمل' ? 'text-green-600' : project.status === 'قيد التنفيذ' ? 'text-[#fa9e1b]' : 'text-blue-600'}`}>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-semibold text-[#31124b]">{project.category?.name || project.category || 'غير مصنف'}</span>
                </div>
                
                <div className="flex items-center justify-between">
                  <span className="text-gray-600">تاريخ البدء</span>
                  <span className="font-semibold text-[#31124b]">{project.startDate || '-'}</span>
                </div>
                
                <div className="flex items-center justify-between">
                  <span className="text-gray-600">تاريخ الانتهاء</span>
                  <span className="font-semibold text-[#31124b]">{project.endDate || '-'}</span>
                </div>
              </div>
