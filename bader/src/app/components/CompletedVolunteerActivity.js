...
export default function CompletedVolunteerActivity({ completedProjects }) {
  ...
  useEffect(() => {
    if (completedProjects && completedProjects.length > 0) {
      const projectsByMonth = groupProjectsByMonth(completedProjects);
      setChartData(projectsByMonth);
    }
  }, [completedProjects]);
  ...
  const groupProjectsByMonth = (projects) => {
    ...
    projects.forEach((project) => {
      const projectDate = project.reportedAt;
