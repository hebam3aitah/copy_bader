  ...
  return (
    ...
    >
              ...
              {showShareMenu && (
                ...
                >
                  <div className="mb-3 pb-2 border-b border-gray-100">
                    <span className="text-sm font-semibold text-[#31124b]">مشاركة المشروع</span>
                  </div>
      ...
      <div className="p-5 flex-grow flex flex-col">
        <h3 className="text-xl font-bold text-[#31124b] mb-3 line-clamp-1">{title}</h3>
        <p className="text-gray-600 mb-5 text-sm line-clamp-2">{description}</p>
        
        {/* شريط التقدم مع نسبة مئوية */}
        <div className="mt-auto">
          <div className="flex justify-between items-center mb-2">
            <span className="text-sm font-medium text-gray-500">نسبة الإنجاز</span>
            <span className="text-sm font-bold">{status === 'مكتمل' ? '100%' : `${progress || 0}%`}</span>
          </div>
      ...
      <div className={`h-1 w-full transition-colors duration-300 ${
        ...
        status === 'قيد التنفيذ' ? 'bg-[#fa9e1b]' : 'bg-blue-500'
      }`}></div>
    </motion.div>
  );
};
